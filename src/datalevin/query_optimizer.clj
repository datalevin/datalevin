      ...
      (fn [g e {:keys [free]}]
        (reduce
          (fn [g {:keys [attr var]}]
            (if (es var)
              (-> g
                 (update-in [e :links] conjv (mk-link :ref var nil nil attr))
                 (update-in [var :links] conjv (mk-link :_ref e nil nil attr)))
              g))
          g free))
      graph graph)))
        ...
        (reduce
          (fn [g [[e1 k1] [e2 k2]]]
            (let [attrs {e1 k1 e2 k2}]
              (-> g
                  (update-in
                    [e1 :links] conjv (mk-link :val-eq e2 v attrs nil))
                  (update-in
                    [e2 :links] conjv (mk-link :val-eq e1 v attrs nil)))))
