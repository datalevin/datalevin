...
(ns ^:no-doc datalevin.query.optimizer.plan-build
  ...
  (:require
   ...
   [datalevin.query.optimizer.estimates :refer [access-variable-domain-estimates cap-output-count capped-tuple-count-sum conventional-access-cost count-init-follows count-init-follows-summary-cached count-only-output-cap count-or-join-follows estimate-hash-join-cost estimate-link-cost estimate-round estimate-row-evaluation-cost estimated-fragment-join-cost estimated-plan-size final-plan-size incoming-link-counts materialize-pattern-with-cost materialized-output-cost n-costly-preds over-count-cap projected-pattern-materialization-cost recount-node relation-size reset-cost-planning scaled-row-estimate selective-anchor-cost selective-rule-anchor-max-inline-size selective-rule-anchor-max-output-size selective-rule-anchor-max-rule-size selective-rule-anchor-min-plan-size top-k-enforcer-cost variable-ref-count verified-non-empty-size]]
   [datalevin.query.optimizer.sampling :refer [-sample access-sample-relation estimated-access-sample-stage-cost estimated-late-input-size sample-context-size sample-relation-projection-count sample-safe-residual? sample-stage-output-cap sampled-access-cardinality sampled-access-output]])
  (:import
...
(declare access-clause-deps access-clause-ready? aid build-plan find-index plan-not-joins plans)
