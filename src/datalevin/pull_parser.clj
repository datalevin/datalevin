...
(defn- check-limit [db pull-attr limit]
  (check (or (and (number? limit) (pos? ^long limit)) (nil? limit)) "(positive-number | nil)" limit)
...
(defn parse-map-spec [db attr-spec pattern]
  (let [pull-attr (parse-attr-spec db attr-spec)]
    ...
    (cond
      ...
      (number? pattern)
      (do
        (check (pos? ^long pattern) "(positive-num | ...)" {attr-spec pattern})
