(ns ^:no-doc datalevin.kv.txlog
  ...
  (:require
   ...
   [datalevin.kv.snapshot :refer [copy-dir-contents! list-snapshot-entries
                                  ...
                                  snapshot-meta-file-name
                                  snapshot-previous-slot snapshot-root-dir
...
(declare txlog-retention-state-map delete-txlog-segment! txlog-snapshot-floor-state txlog-recovery-context txlog-force-sync! txlog-config-enabled? txlog-rollout-mode txlog-rollout-watermarks ensure-txlog-ready! txlog-write-path-enabled? write-txn-open? txlog-watermarks-map txlog-update-snapshot-floor-state! txlog-clear-snapshot-floor-state! txlog-update-replica-floor-state! txlog-clear-replica-floor-state! txlog-pin-backup-floor-state! txlog-unpin-backup-floor-state! reset-txlog-runtime-for-snapshot! txlog-mark-recovery-source! persisted-payload-floor-lsn persisted-runtime-floor-lsn start-snapshot-scheduler-hook! stop-snapshot-scheduler-hook! create-snapshot-now! txlog-watermarks close-failed-open! recover-from-snapshot-open! txlog-vector-floor-state txlog-backup-pin-floor-state txlog-replica-floor-state read-commit-marker-state txlog-copy-with-backup-pin!
         ...
         txlog-replica-floor-state txlog-recovery-context
         reset-txlog-runtime-for-snapshot! txlog-mark-recovery-source!
         persisted-payload-floor-lsn persisted-runtime-floor-lsn
         txlog-watermarks txlog-rollout-mode txlog-rollout-watermarks
         txlog-config-enabled? txlog-write-path-enabled? write-txn-open?
         txlog-watermarks-map txlog-snapshot-floor-state
         txlog-update-snapshot-floor-state! txlog-clear-snapshot-floor-state!
         txlog-update-replica-floor-state! txlog-clear-replica-floor-state!
         txlog-pin-backup-floor-state! txlog-unpin-backup-floor-state!)

(defonce ^:private snapshot-scheduler-hooks*
  (atom {:start nil :stop nil}))

(defn set-snapshot-scheduler-hooks!
  "Register the snapshot scheduler start/stop functions."
  [start-fn stop-fn]
  (reset! snapshot-scheduler-hooks* {:start start-fn :stop stop-fn})
  nil)

(defn- start-snapshot-scheduler-hook!
  [lmdb]
  (when-let [f (:start @snapshot-scheduler-hooks*)]
    (f lmdb)))

(defn- stop-snapshot-scheduler-hook!
  [info-v]
  (when-let [f (:stop @snapshot-scheduler-hooks*)]
    (f info-v)))
  ...
  (let [info-v (if (and (instance? clojure.lang.IDeref db-or-info)
    ...
    (when (some? info-v)
      (when-let [state (:txlog-state @info-v)]
        (stop-snapshot-scheduler-hook! info-v)
        (try
...
(def txlog-rollout-mode-values #{:active :rollback})
