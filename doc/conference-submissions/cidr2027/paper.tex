...
direction. Datalevin \cite{datalevin} addresses these challenges with a layered strategy:
index-maintained base-cardinality counts from nested entity--attribute--value (EAV) and
attribute--value--entity (AVE) indexes,
...
they do not predict wall-clock time. The optimizer chooses the lowest
...
which maps one key to an ordered value list. This forms a nested B$^+$ tree
with page-level prefix compression. Internal nodes carry subtree
...
the same evidence. A separate Exact reference, described below, calibrates
Full without entering the layer ablation.
...
$\alpha\in\{0,0.4,1.6\}$. The frozen point ($m=250$, $k=400$, $\alpha=1.6$)
...
($S_{q,s,p}\geq 10$). Because the denominator is a Full median rather than
...
samples the live index during optimization \cite{erlingVirtuoso}. Both systems
tie their evidence to triple indexes or graph structure.

Index-based join sampling has been evaluated on JOB \cite{leis2017cardinality};
learned estimators target correlated joins \cite{kipf2019learned}. Datalevin
instead uses query-time estimates without training or external model state,
and our ablations isolate counts, sampling, and regularization.

Robust optimization targets predictable performance under uncertainty
\cite{babcock2005robust}. Datalevin derives its uncertainty evidence directly
from the indexes used for execution.
...
selectivity and directional fanout, and prior regularization keeps
noise from crossing damaging plan boundaries. Hiding counts caused 30
timeouts; disabling sampling caused 80; and shrinkage lowered Raw's P99 from
$12.26\times$ to $2.64\times$ on identical ranks. With exact cardinalities,
the same optimizer's median paired Full/Exact execution-time ratio was
$1.086\times$ [1.044, 1.160], exposing remaining estimator headroom.
...
not a universal failure probability. Future work should focus sampling near
