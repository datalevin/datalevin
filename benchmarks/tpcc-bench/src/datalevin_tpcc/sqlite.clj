...
(defn payment!
  ...
  (try
    (let [[w-ytd w-name] (q1 conn
                            "SELECT w_ytd, w_name FROM warehouse WHERE w_id = ?"
                            w)
          [d-ytd d-name] (q1 conn
                            "SELECT d_ytd, d_name FROM district WHERE d_w_id = ? AND d_id = ?"
                            w d)
          cust  (if by-name?
...
(defn- do-txn
  "Run one transaction. The caller generates `input` and owns the timing so
  that the measured latency includes waiting for the SQLite writer lock but
  excludes input generation, matching the other backends."
  [conn input type]
  (case type
                      ...
                      (let [r (Random. (+ seed ti 1))]
                        (dotimes [_ n]
                          (let [type     (pick-type r)
                                input    (gen-input r opts type)
                                ;; SQLite has a single writer, so write
                                ;; transactions serialize on one lock. Waiting
                                ;; for that lock is part of the latency.
                                [res ms] (with-write-lock
                                           write-lock
                                           #(do-txn cnn input type))]
